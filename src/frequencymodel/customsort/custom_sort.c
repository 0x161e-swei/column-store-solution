...
static inline void pointer_swap(char* a, char* b) {
	partition_struct* __a = (partition_struct *) a;
	partition_struct* __b = (partition_struct *) b;
	if(__a->next_neighbor){
		__a->next_neighbor->prev_neighbor = __a;
	}
	if(__a->prev_neighbor){
		__a->prev_neighbor->next_neighbor = __a;
	}
	if(__b->next_neighbor){
		__b->next_neighbor->prev_neighbor = __b;
	}
	if(__b->prev_neighbor) {
		__b->prev_neighbor->next_neighbor = __b;
	}
}
/*
 * static inline void pointer_swap(char* a, char* b) {
