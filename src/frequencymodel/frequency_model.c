...
void build_frequency_model_api(data* data,const int size, const int* type, const int* first, const int* second,frequency_model *fm, partition_struct *ps) {
	...
	for(int i = 0, j=0; i < (int)data->size; i+=fm->block_size, j++){

		high_val[j] = data->array[i];
		//initialize the partition representation

		ps[j].max_val = data->array[i];
		ps[j].max_block = j;
		ps[j].min_block = j;
		ps[j].score = 0.0;
		ps[j].partition_static_cost = 0.0;

		if(j+1 < fm->histogram_size){
			ps[j].next_neighbor = &ps[j+1];
		} else {
			ps[j].next_neighbor = NULL;
		}

		if(j != 0) {
			ps[j].prev_neighbor = &ps[j-1];
		} else {
			ps[j].prev_neighbor = NULL;
		}
	}

	//catch the last block in case data->size % fm->block_size != 0
	high_val[fm->histogram_size-1] = data->array[data->size-1];
	for(int i = 0; i < size; i++) {
		...
		//range queries
		//range_query(data,high_val,work,fm,threads);

		//update
		//FIXME: Does not work with ghost values. Need an update insert counter for that
		//update(data,high_val,work,fm,threads);
	}
...
void partition_data(const int* data_in, size_t size,const int* type, const int* first, const int* second, size_t work_size, Partition_inst *out){
	...
	part_size = 0;
	while(pointer) {
		if(pointer->max_val > 0){
			out->pivots[part_size] = pointer->max_val;
			part_size++;
		}
		pointer = pointer->next_queue;
	}
	out->pivots[part_size] = data.array[data.size-1];
